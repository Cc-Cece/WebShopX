...
> 实施规范化补充（2026-07-26）：背包接口继续使用 `containerItems`；商城、市场、拍卖、
> 上架、订单和后台页面则从记录自身保存的 `itemMetaJson.containerItems` 读取同构的只读
> 嵌套摘要。摘要在 `ItemSnapshotCodec` 创建快照时一并冻结，不回查玩家当前背包。
> 旧快照没有嵌套摘要时安全降级为不显示预览，不根据材质猜测或伪造内容。递归边界与背包
> 预览一致：最大深度 8、每节点最多 64 项。
>
> 实施状态：计划内现有页面均已接入统一组件；快速预览由 `SnapshotItemIcon` 统一承载，
> 完整预览由 `SnapshotNestedContents` 承载，并在同一区域使用 breadcrumb 浏览嵌套节点。
> 未新增 Mailbox 或其他业务页面。自动化测试与构建状态见实施交付记录。
>
> 交互补充（2026-07-26）：容器预览中的非空槽位均可点击查看对应物品的完整信息；
> 普通附魔、附魔书存储附魔、lore、自定义模型等复用统一详情展示和中文附魔名称。
> 点击嵌套容器时，详情内继续显示其下一层内容，不创建无限层叠弹窗。
