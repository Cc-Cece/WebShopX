import {
  normalizeApiBaseUrl as normalizeApiBaseUrlCore,
  resolveApiUrl as resolveApiUrlCore,
  formatCurrency as formatCurrencyCore,
  formatDateTime as formatDateTimeCore,
  toDateTimeLocalValue as toDateTimeLocalValueCore,
  parseDateTimeValue as parseDateTimeValueCore,
  normalizeMarketAlgorithmGlossary as normalizeMarketAlgorithmGlossaryCore,
  parseAlgorithmParamsJson as parseAlgorithmParamsJsonCore,
  parseOptionalPositiveWhole as parseOptionalPositiveWholeCore,
  normalizeMaterialKey as normalizeMaterialKeyCore,
  getLocalizedMaterialName as getLocalizedMaterialNameCore,
  buildAdminOrderDigest as buildAdminOrderDigestCore,
  buildAdminMarketDigest as buildAdminMarketDigestCore,
  FALLBACK_MARKET_ALGORITHM_GLOSSARY as FALLBACK_MARKET_ALGORITHM_GLOSSARY_CORE,
} from "./core-logic.js";
...
const API_BASE_URL = normalizeApiBaseUrlCore(RUNTIME_CONFIG.apiBaseUrl || "");

function normalizeApiBaseUrl(value) {
  return normalizeApiBaseUrlCore(value);
}

function resolveApiUrl(path) {
  return resolveApiUrlCore(path, API_BASE_URL);
}
...
const elements = {
  ...
  adminConfirmDialog: document.getElementById("adminConfirmDialog"),
  adminConfirmTitle: document.getElementById("adminConfirmTitle"),
  adminConfirmBody: document.getElementById("adminConfirmBody"),
  adminConfirmCancel: document.getElementById("adminConfirmCancel"),
  adminConfirmOk: document.getElementById("adminConfirmOk"),
...
function notify(message, tone = "info", durationMs = 3200) {
  ...
  const text = localizeDisplayText(message);
  if (window.mdui && typeof window.mdui.snackbar === "function") {
    window.mdui.snackbar({ message: text, placement: "top", closeable: normalized === "error" });
    return;
  }
...
function setGlobalBusy(busy) {
  if (!elements.adminProgress) {
    return;
  }
  elements.adminProgress.style.display = busy ? "block" : "none";
}
...
function getSelectItems(select) {
  ...
  return Array.from(select.querySelectorAll("mdui-menu-item"));
}
...
function setSelectItems(select, items, selectedValue = "") {
  ...
  items.forEach((item) => {
    ...
    setNodeText(option, item.label ?? item.value ?? "");
...
function closeAdminConfirm(result) {
  if (elements.adminConfirmDialog) {
    elements.adminConfirmDialog.open = false;
  }
  if (typeof adminConfirmResolver === "function") {
    const resolve = adminConfirmResolver;
    adminConfirmResolver = null;
    resolve(Boolean(result));
  }
}

function askAdminConfirm(title, message) {
  if (!elements.adminConfirmDialog) {
    return Promise.resolve(true);
  }
  if (elements.adminConfirmTitle) {
    setNodeText(elements.adminConfirmTitle, title || "确认操作");
  }
  if (elements.adminConfirmBody) {
    setNodeText(elements.adminConfirmBody, message || "请确认是否继续执行。");
  }
  elements.adminConfirmDialog.open = true;
...
function setStatus(text, stateName) {
  ...
  elements.statusChip.variant = stateName === "online" ? "assist" : "elevated";
}
...
function switchTab(tabName, skipHistory = false) {
  ...
  tabs.forEach((tab) => {
    ...
    if (tab.tagName.toLowerCase().startsWith("mdui-button")) {
      tab.variant = active ? "filled" : "tonal";
    }
  });
...
document.addEventListener("click", (event) => {
  ...
  const clickable = path.find((node) => node && node.dataset && node.dataset.tabTarget);
  if (!clickable) {
    return;
  }
  switchTab(clickable.dataset.tabTarget);
});
...
async function fetchWithProgress(resource, options = {}) {
  beginNetworkRequest();
  try {
...
async function loadCurrencyMeta() {
  try {
    const payload = await apiAdmin("/api/meta/currency", { method: "GET" });
...
function applyCurrencyMetaToUi() {
  ...
  const updateSelect = (select) => {
    ...
    getSelectItems(select).forEach((option) => {
...
function formatCurrency(amount, currency) {
  const locale = I18N ? I18N.getIntlLocale() : "zh-CN";
  return formatCurrencyCore(
    amount,
    currency,
    {
...
function parseDateTimeValue(value) {
  return parseDateTimeValueCore(value, state.timeZone);
}
...
function formatDateTime(value) {
  return formatDateTimeCore(value, {
    locale: I18N ? I18N.getIntlLocale() : "zh-CN",
    timeZone: state.timeZone,
  });
}

function toLocalInput(value) {
  return toDateTimeLocalValueCore(value, { timeZone: state.timeZone });
}
...
function localizeOrderStatusOptions() {
  ...
  getSelectItems(elements.orderStatus).forEach((option) => {
...
function buildAdminOrderDigest(orders) {
  return buildAdminOrderDigestCore(orders);
}
...
function buildAdminMarketDigest(listings) {
  return buildAdminMarketDigestCore(listings);
}
...
function normalizeMaterialKey(text) {
  return normalizeMaterialKeyCore(text);
}
...
function getLocalizedMaterialName(material) {
  ...
  return getLocalizedMaterialNameCore(aliasKey || key, state.materialMap, I18N ? I18N.getLocale() : "zh-CN")
    || humanizeMaterial(aliasKey || key);
}
...
async function ensureMaterialAllowList() {
  ...
  state.materialAllowPromise = fetchWithProgress(resolveApiUrl("/api/meta/materials"))
    .then((response) => {
      if (!response.ok) {
...
async function ensureMaterialMap() {
  ...
  state.materialMapPromise = ensureMaterialAllowList()
    .then(() => {
      ...
      return fetchWithProgress(`i18n/materials/${I18N.getLocale()}.json`).then((response) => {
        if (!response.ok) {
...
function normalizeMarketAlgorithmGlossary(raw) {
  return normalizeMarketAlgorithmGlossaryCore(raw || {}, FALLBACK_MARKET_ALGORITHM_GLOSSARY_CORE);
}
...
async function ensureMarketAlgorithmGlossary() {
  ...
  state.marketAlgorithmGlossaryPromise = (async () => {
    for (const path of candidates) {
      try {
        const response = await fetchWithProgress(path, { cache: "no-cache" });
        if (!response.ok) {
          continue;
        }
...
function parseAlgorithmParamsJson(raw) {
  return parseAlgorithmParamsJsonCore(raw);
}
...
function populateProductDynamicAlgorithmSelect() {
  ...
  const items = catalog.map((item) => ({ value: item.id, label: item.label || item.id }));
  if (!matched && current) {
    items.push({ value: current, label: current });
  }
  setSelectItems(
    elements.productDynamicAlgorithm,
    items,
    matched ? current : (current || catalog[0].id)
  );
}
...
function parseOptionalPositiveWhole(raw) {
  return parseOptionalPositiveWholeCore(raw);
}
...
async function resetProductLimit(product) {
  ...
  const confirmed = await askAdminConfirm("确认清空限购", `确认清空商品 ${product.sku} 的所有玩家限购记录吗？`);
  if (!confirmed) {
    return;
  }
  ...
  const rows = filtered.map((product) => {
    ...
    editBtn.addEventListener("click", () => {
      ...
      if (elements.productDynamicAlgorithm) {
        ...
        const matched = getSelectItems(elements.productDynamicAlgorithm).some(
          (option) => String(option.value || "").trim().toUpperCase() === dynamicAlgorithm
        );
        if (!matched && dynamicAlgorithm) {
          const customOption = document.createElement("mdui-menu-item");
...
function populateAdminTemplates() {
  ...
  const items = [{ value: "", label: "自定义" }];
  (state.adminMeta?.templates || []).forEach((template) => {
    items.push({ value: template.key, label: template.label });
  });
  setSelectItems(elements.adminManagerTemplate, items, current || "");
}
...
if (elements.adminConfirmCancel) {
  elements.adminConfirmCancel.addEventListener("click", () => closeAdminConfirm(false));
}
if (elements.adminConfirmOk) {
  elements.adminConfirmOk.addEventListener("click", () => closeAdminConfirm(true));
}
if (elements.adminConfirmDialog) {
  ["close", "closed"].forEach((eventName) => {
    elements.adminConfirmDialog.addEventListener(eventName, () => {
      if (typeof adminConfirmResolver === "function") {
