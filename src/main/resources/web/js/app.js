...
async function runListingAction(path, listingId, successMessage) {
  if (!requireAuth()) {
    return;
  }
  try {
...
function bindCoreEvents() {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (action === "market-pause") {
      void runListingAction("/api/market/pause", clickable.dataset.listingId, "已暂停上架");
      return;
    }

    if (action === "market-resume") {
      void runListingAction("/api/market/resume", clickable.dataset.listingId, "已恢复上架");
      return;
    }

    if (action === "market-unlist") {
      void runListingAction("/api/market/unlist", clickable.dataset.listingId, "已下架");
      return;
    }
