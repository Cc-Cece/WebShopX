...
final class InventoryOperationService {
    ...
    return databaseManager.withConnection(connection -> {
      try (PreparedStatement statement = connection.prepareStatement("""
        ...
        try (ResultSet rows = statement.executeQuery()) {
          ...
          return new Existing(rows.getString(1), rows.getString(2), (Long) rows.getObject(3),
  ...
  void begin(
      long userId, String key, String action, int slot, Integer containerSlot,
  ...
  private void update(
      long userId, String key, String state, Long referenceId, String resultJson, String errorCode) {
    databaseManager.inTransaction(connection -> {
      try (PreparedStatement statement = connection.prepareStatement("""
  ...
  private void validateKey(String key) {
    if (key == null || key.isBlank() || key.trim().length() > 96) {
      throw new ServiceException("invalid_idempotency", "Idempotency key is required and must be <= 96 characters");
    }
  }

  record Existing(String action, String state, Long referenceId, String resultJson, String errorCode) {}
}
